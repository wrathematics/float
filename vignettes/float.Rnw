%\VignetteIndexEntry{Introducing float: Single Precision Floats for R}
...
\mytitle{Introducing {float}: Single Precision Floats for R}
\mysubtitle{}
\myversion{0.1-0}
\myauthor{
\centering
...
enough for a particular task.  The \textbf{float} package~\cite{float} 
...
The linear algebra operations in the \textbf{float} package are handled by the 
...
times of using tuned BLAS/LAPACK with \textbf{float}, you will need to choose 
an implementation and install it.  Typical implementations include 
\textbf{OpenBLAS}~\cite{OpenBLAS}, Intel \textbf{MKL}~\cite{mkl}, AMD 
\textbf{ACML}~\cite{acml}, and \textbf{Atlas}~\cite{atlas}, and Apple's 
\textbf{Accelerate}~\cite{accelerate}. You can read more about using different 
...
\textbf{float} package from source.
...
from integer/numeric to float using \code{fl()}:
...
This will of course require 1.5x the memory of the input matrix. For workflows 
requiring many operations, the memory savings will still be substantial. At 
this time, we do not have a reader, so casting is the best way to go. You can 
also cast a float to a numeric (double precision) matrix via \code{dbl()}.

For testing or other cases where random matrices are needed (e.g., PCA via 
random normal projections~\cite{halko2011finding}), 
...
##             [,1]      [,2]
## [1,] 0.640310585 0.2325505
## [2,] 0.009495757 0.6660838
flrunif(5, min=10, max=20)
## # A float32 vector: 5
## [1] 15.14251 16.93591 15.44975 12.82734 19.23433
\end{lstlisting}
...
This is conceptually similar to first creating \code{n} random values and then 
casting them over to floats, but more memory efficient. The function processes 
the generator data in 4KiB chunks (for double precision generators).
...
like. But the output will be determined by the highest precision. The arithmetic 
itself will be carried out in the highest possible precision.  So adding a float 
matrix with a double matrix is really just adding 2 double matrices and 
...
To create a \code{float32} object, use \code{methods::new()}:

\begin{lstlisting}[language=rr]
Data = 1:3
x = new("float32", Data=Data)
x
## # A float32 vector: 3
...
So when creating new functionality not provided by existing \textbf{float} 
package methods, you will probably have to move to compiled code.


\subsection{Compiled Code}

Using 32-bit floats from \textbf{float} in compiled code is not terribly 
...
S4 class: \code{new("float32", Data=ret)}.
\end{itemize}

One can access the data with the following macro:
...
double precision and \code{INTEGER} for ints. There is no Rcpp-like idiom for 
using floats (\code{NumericVector} and \code{NumericMatrix}) at this time.

Here's a basic example of how one would create a new function \code{add1()} in 
R (ignoring that we could just do \code{x+1}).
...
Note that using \code{INTEGER(ret)[0]} instead of \code{FLOAT(ret)[0]} is not 
correct. That would cast the value to an integer, then once put in the 
\code{float32} class, that integer value would be treated as the memory of a 
\code{float}. If that doesn't make sense, try modifying the above to the wrong 
...
implementation of covariance, namely \code{covar()} from the \textbf{coop} 
package~\cite{coop}:
