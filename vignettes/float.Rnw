%\VignetteIndexEntry{Introducing float: 32-Bit Floats for R}
...
\mytitle{Introducing {float}: 32-Bit Floats for R}
\mysubtitle{}
\myversion{0.1-2}
\myauthor{
\centering
...
add \textbf{float} to the \code{LinkingTo} list in the package DESCRIPTION file. 
